                    ...
                    int method)
{
    ...
    for (col = 0; col < cols; col++) {
        ...
        n = Rast_get_c_value((char *)n_array + n_offset, CELL_TYPE);
        sum = Rast_get_d_value((char *)sum_array + offset, rtype);
        sumsq = Rast_get_d_value((char *)sumsq_array + offset, rtype);
    ...
    case METHOD_MEAN: /* (sum / n) */
        ...
        for (col = 0; col < cols; col++) {
            ...
            double sum = Rast_get_d_value(
                (char *)point_binning->sum_array + offset, rtype);
    ...
    if (point_binning->bin_coordinates) {
        for (col = 0; col < cols; col++) {
            ...
            double sum_x = Rast_get_d_value(
                (char *)point_binning->x_array + offset, rtype);
            double sum_y = Rast_get_d_value(
                (char *)point_binning->y_array + offset, rtype);
