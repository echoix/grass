...
def main():
    ...
    # Test input values
    if abs(dip) >= 90:
        gs.fatal(_("dip must be between -90 and 90."))

    if az < 0 or az >= 360:
        gs.fatal(_("azimuth must be between 0 and 360"))
    ...
    gs.run_command("r.support", map=name, history="")
    ...
    gs.message(t.substitute(name=name, ea=ea, no=no, el=el, dip=dip, az=az))
